					...
					if(($num_rowsx = mysql_num_rows($resultx))>0)
					{
						...
						while($rowgroup = mysql_fetch_array($resultx))
						{
							...
							if(verify_group($rowgroup["id"],$user["id"]))
							{
								...
								$date = $exp[0];
								if($user["id"]==$rowgroup["sender"] && $rowgroup["view_sender"]=="no")
									$imagenew = $imgnew;
								else if(verify_group_valid($rowgroup["id"],$user["id"]))
									$imagenew = $imgnew;
								else
									$imagenew ="";
            ...
            if($result = mysql_query($query))
            {
                if(($num_rows = mysql_num_rows($result))>0)
                {
                    ...
                    while($rows = mysql_fetch_array($result))
                    {
                        ...
                        $date = $exp[0];
						if($user["id"]==$rows["sender"] && $rows["view_sender"]=="no")
							$imagenew = $imgnew;
						else if($user["id"]==$rows["receiver"] && $rows["view_receiver"]=="no")
							$imagenew = $imgnew;
						else
							$imagenew ="";
            ...
            if($result = mysql_query($querys))
            {
                if(($num_rows = mysql_num_rows($result))>0)
                {
                    ...
                    while($rows = mysql_fetch_array($result))
                    {
                        ...
                        $exp = explode(" ",$rows["date"]);
                        $date = $exp[0];
						if($user["id"]==$rows["sender"] && $user["view_sender"]=="no")
							$imagenew = $imgnew;
						else if($user["id"]==$rows["receiver"] && $user["view_receiver"]=="no")
							$imagenew = $imgnew;
						else
							$imagenew ="";
						if(empty($rows["receiver"]) && $rows["group_task"]=="yes")
